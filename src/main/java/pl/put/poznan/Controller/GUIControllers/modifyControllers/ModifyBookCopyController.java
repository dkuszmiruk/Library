...
public class ModifyBookCopyController {
    ...
    @FXML
    public Button modifyBookCopyButton;

    @FXML
    public void modifyBookCopy() {
        bookCopyFx.setHowLong(Integer.parseInt(howLongField.getText()));
        bookCopyFx.setSection(sectionField.getText());
        bookCopyFx.setBookFx(getBookFxObjectProperty());
        try {
            bookCopyDAO.modifyBookCopy(convertToBookCopy(bookCopyFx));
            newStage.hide();
            bookCopiesController.refreshTableBookCopy();
            DialogUtils.informationDialog("Egzemplarz został zmodyfikowany");
            newStage.close();
        } catch (ApplicationException e) {
            DialogUtils.errorDialog("Nie udało się zmodyfikować egzemplarza", e.getMessage());
        }
    }

    @FXML
    public void closeModifyBookCopy() {
        newStage.close();
    }
    ...
    private void initialize(){
        ...
        setComboBox();
        modifyBookCopyButton.disableProperty().bind(howLongField.textProperty().isEmpty().or(sectionField.textProperty().isEmpty()).or(bookFxObjectProperty.isNull()));
    ...
    private void setFields() {
        howLongField.setText(String.valueOf(bookCopyFx.getHowLong()));
        sectionField.setText(bookCopyFx.getSection());
        bookComboBox.getSelectionModel().select(bookCopyFx.getBookFx());
        onBookComboBox();
    }
